...
APPLY_CHANGES = resolve_optional_bool("ONE_PIECE_MIGRATION_APPLY", default=DEFAULT_APPLY_CHANGES)
...
ALLOW_CROSS_VERSION_MATCH = resolve_optional_bool(
    "ONE_PIECE_MIGRATION_ALLOW_CROSS_VERSION",
    default=DEFAULT_ALLOW_CROSS_VERSION_MATCH,
)
PRUNE_LEGACY = resolve_optional_bool("ONE_PIECE_MIGRATION_PRUNE_LEGACY", default=DEFAULT_PRUNE_LEGACY)
...
def has_official_one_piece_url(value):
    normalized = clean_text(value).lower()
    return "onepiece-cardgame.com" in normalized


def is_variant_card(card):
    source_code = normalize_source_card_id(card.source_card_id)
    deck_code = normalize_source_card_id(card.deck_key or card.source_card_id)
    if source_code != deck_code:
        return True
    return bool(re.search(r"-P\d+$", source_code, flags=re.IGNORECASE))
...
def is_official_like_card(card, detail):
    if detail and has_value(detail.qa):
        return True
    if has_value(card.description) and clean_text(card.description).startswith("Set: "):
        return True
    if detail and has_value(detail.notes) and "Alternative illustration of" in clean_text(detail.notes):
        return True
    return False


def score_one_piece_card(card, detail):
    score = 0

    if detail and has_value(detail.qa):
        score += 500
    if detail and has_official_one_piece_url(detail.attribute_image):
        score += 140
    if has_official_one_piece_url(card.image_url):
        score += 120
    if has_value(card.description) and clean_text(card.description).startswith("Set: "):
        score += 50
    if detail and has_value(detail.notes) and "Alternative illustration of" in clean_text(detail.notes):
        score += 40
    if has_value(card.set_name) and re.search(r"\[[A-Z0-9-]+\]\s*$", clean_text(card.set_name)):
        score += 20
    ...
    for value in (
        detail.attribute_name if detail else None,
        detail.attribute_image if detail else None,
        detail.family if detail else None,
        detail.ability if detail else None,
        detail.counter if detail else None,
        detail.trigger if detail else None,
        detail.notes if detail else None,
    ):
        if has_value(value):
            score += len(clean_text(value))

    for value in (card.cost, card.ap, detail.power if detail else None):
...
def is_significantly_better(current_card, current_detail, candidate_card, candidate_detail):
    current_score = score_one_piece_card(current_card, current_detail)
    candidate_score = score_one_piece_card(candidate_card, candidate_detail)
    ...
    return candidate_score >= current_score + 20
...
def choose_best_candidate(candidates, detail_by_card_id):
    ...
    non_variant_candidates = [card for card in candidates if not is_variant_card(card)]
    if non_variant_candidates:
        candidates = non_variant_candidates

    return max(
        candidates,
        key=lambda card: (score_one_piece_card(card, detail_by_card_id.get(card.id)), -card.id),
    )
...
def find_target_card(current_card, detail_by_card_id, indexes):
    ...
    for match_type, candidates in candidate_groups:
        candidates = [
            ...
            if is_official_like_card(candidate, detail_by_card_id.get(candidate.id))
        ]
...
def build_reference_mapping(db, one_piece_tgc_id):
    one_piece_cards = db.query(Card).filter(Card.tgc_id == one_piece_tgc_id).all()
    detail_by_card_id = {
        detail.card_id: detail
        for detail in db.query(OnePieceCard)
        .join(Card, Card.id == OnePieceCard.card_id)
        .filter(Card.tgc_id == one_piece_tgc_id)
        .all()
    }

    official_like_count = sum(
        1
        for card in one_piece_cards
        if is_official_like_card(card, detail_by_card_id.get(card.id))
    )
    if official_like_count == 0:
        raise ValueError(
            "No official-like One Piece cards were found in the database. "
            "Run backend/populate_one_piece_official.py first."
        )

    indexes = build_indexes(one_piece_cards)
    remap_plan = {}
...
def main():
    ...
    try:
        ...
        one_piece_tgc = get_one_piece_tgc(db)
        if not one_piece_tgc:
            raise ValueError("One Piece TGC was not found in the database.")

        remap_plan, match_type_counts = build_reference_mapping(db, one_piece_tgc.id)
        ...
        if not APPLY_CHANGES:
            ...
            print('$env:ONE_PIECE_MIGRATION_APPLY="true"')
            print(r'.venv\Scripts\python.exe backend\migrate_one_piece_official_references.py')
    finally:
        db.close()
